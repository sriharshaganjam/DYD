...
from utils_def_1 import embedding_model, run_read_cypher, mistral_request, MISTRAL_MODEL, driver
...
class SlidingWindowMemory:
    """Memory system with sliding window + summarization for long conversations"""

    def __init__(self, recent_messages_count=6, max_context_tokens=1500):
        self.recent_messages_count = recent_messages_count  # Keep last N messages fully
        self.max_context_tokens = max_context_tokens
        self.session_key = 'sliding_window_memory'
    ...
    def initialize_memory(self):
        ...
        if self.session_key not in st.session_state:
            st.session_state[self.session_key] = {
                ...
                'last_summarized_index': 0,  # Track where we last summarized
    ...
    def summarize_old_messages(self, messages: List[Dict], client) -> str:
        """Summarize older messages into compact form"""
        if not messages or len(messages) < 8:
            return ""

        # Messages to summarize (exclude recent ones)
        messages_to_summarize = messages[:-self.recent_messages_count]
        ...
        summary_prompt = """Summarize this conversation history into 2-3 concise bullet points focusing on:
- What topics/courses the student asked about
- Key recommendations given
- Any important context

Conversation:
""" + "\n".join(conversation_text[-10:])  # Last 10 old messages

        try:
            if client:
                summary = mistral_request(
                    client=client,
                    model=MISTRAL_MODEL,
                    messages=[
            ...
            for msg in messages_to_summarize:
                if msg['role'] == 'user':
                    ...
                    if 'prerequisite' in content or 'prereq' in content:
                        topics.add('prerequisites')
                    if 'career' in content or 'job' in content:
                        topics.add('careers')
                    # Extract course codes
                    codes = re.findall(r'[A-Z]{2,4}-?\d{2,3}', msg['content'])
                    topics.update(codes[:3])

            if topics:
                return "Earlier discussed: " + ", ".join(list(topics)[:5])
    ...
    def build_context(self, messages: List[Dict], current_query: str, client=None) -> str:
        ...
        # Student profile (always included - compact)
        if profile['name'] or profile['interests'] or profile['career_goals']:
            ...
            if profile['interests']:
                top_interests = sorted(profile['interests'].items(),
        ...
        if len(messages) > self.recent_messages_count:
            # Generate or retrieve summary of old messages
            if memory['last_summarized_index'] < len(messages) - self.recent_messages_count:
                summary = self.summarize_old_messages(messages, client)
                if summary:
                    memory['conversation_summary'] = summary
                    memory['last_summarized_index'] = len(messages) - self.recent_messages_count

            # Include summary of older messages
            if memory['conversation_summary']:
                context_parts.append(f"EARLIER CONVERSATION:\n{memory['conversation_summary']}")

            # Include recent messages fully
...
def generate_response(query_results: Dict[str, Any], user_input: str, client,
    ...
    """Generate response with sliding window memory and curious questioning"""
        ...
        with driver.session(database=NEO4J_DATABASE) as s:
            result = s.run("""
                ...
                RETURN u.email AS email,
                       ...
                       u.riasec_scores AS all_scores
            """, username=st.session_state.username).single()


    except Exception as e:
        st.error(f"Error loading DB in function file: {e}")
    ...
    # Enhanced system prompt with curiosity
...
CONTEXT STRUCTURE:
- STUDENT PROFILE: Contains verified name, interests, courses discussed
- EARLIER CONVERSATION: Brief summary of older messages (if conversation is long)
- RECENT MESSAGES: Last few exchanges in full detail

INSTRUCTIONS:
1. Use STUDENT PROFILE for personal information
2. Reference EARLIER CONVERSATION for general context about past topics
...
GREETING:
- First message: "Hello [Name], nice to meet you!"
- Don't claim to remember things not in EARLIER CONVERSATION or RECENT MESSAGES
...
- Questions should help you understand what they like/dislike
- Examples:
  * "What aspects of [subject] interest you most?"
  * "Have you had any experience with [topic] before? What did you think?"
  * "Does this sound like something you'd enjoy, or would you prefer something different?"
  * "What kind of career path are you imagining f or yourself?"
  * "Is there a particular area within [field] that excites you?"
  * "What made you interested in [topic]?"

RIASEC SCORES:
- If the student has RIASEC scores, use them to suggest courses/careers that align with their interests
- If no RIASEC info, focus on courses and jobs relevant to their query
these are the top 3 scores {result['all_scores']}

Keep responses 2-3 paragraphs, recommend relevant courses, then ask engaging questions (not too many tho)."""
    ...
    try:
        response = mistral_request(
            ...
            model=MISTRAL_MODEL,
            messages=messages,
            max_tokens=400,  # Increased to allow for questions
            temperature=0.3  # Slight increase for more natural questions
        )
        ...
        if response and not any(response.rstrip().endswith(q) for q in ['?', '? ', '?)']):
            # Add a fallback question if LLM didn't generate one
            fallback_questions = [
